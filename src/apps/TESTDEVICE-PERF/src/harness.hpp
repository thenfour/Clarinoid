...
static const UQFOperation kOps_UQF[] = {
    {"sqrt", 32, 16, &sqrt, kSqrtQ32Variants,
     (uint32_t)(sizeof(kSqrtQ32Variants) / sizeof(kSqrtQ32Variants[0]))},
    {"sqrt", 24, 12, &sqrt,
     (const VariantUQF[]){{"restoring (int)", &sqrt_q24_rest_wrap},
                          {"float-assisted", &sqrt_q24_float_wrap}},
     2u},

    {"tanh", 31, 31, &tanhf, kTanhQ32Variants,
     (uint32_t)(sizeof(kTanhQ32Variants) / sizeof(kTanhQ32Variants[0]))},
};
static const uint32_t kNumOps_UQF = sizeof(kOps_UQF) / sizeof(kOps_UQF[0]);

// float op registry
static const FloatOperation kOps_Float[] = {
    {"sqrtf", 0.0f, 1.0e8f, &sqrtf, kSqrtFVariants,
     (uint32_t)(sizeof(kSqrtFVariants) / sizeof(kSqrtFVariants[0]))},
    {"tanh-around0", -0.05f, 0.05f, &tanhf, kTanhFVariants,
     (uint32_t)(sizeof(kTanhFVariants) / sizeof(kTanhFVariants[0]))},
    {"tanh-0.5", 0.45f, 0.55f, &tanhf, kTanhFVariants,
     (uint32_t)(sizeof(kTanhFVariants) / sizeof(kTanhFVariants[0]))},
    {"tanh-before1", 0.9f, 1, &tanhf, kTanhFVariants,
     (uint32_t)(sizeof(kTanhFVariants) / sizeof(kTanhFVariants[0]))},
    {"tanh-after1", 1, 1.2f, &tanhf, kTanhFVariants,
     (uint32_t)(sizeof(kTanhFVariants) / sizeof(kTanhFVariants[0]))},
    {"tanh-below-n2", -16, -2, &tanhf, kTanhFVariants,
     (uint32_t)(sizeof(kTanhFVariants) / sizeof(kTanhFVariants[0]))},
    {"tanh-above-p2", 2, 16, &tanhf, kTanhFVariants,
     (uint32_t)(sizeof(kTanhFVariants) / sizeof(kTanhFVariants[0]))},
    // {
    //     "recip",
    //     0.001f,
...
static const uint32_t kNumOps_Float =
    sizeof(kOps_Float) / sizeof(kOps_Float[0]);
...
static void print_markdown_summaries() {
  ...
  for (uint32_t i = 0; i < g_rows_uqf_count; ++i) {
    ...
    for (uint32_t k = 0; k < i; ++k)
      if (g_rows_uqf[k].op_name == name_i && g_rows_uqf[k].F == Fi &&
          g_rows_uqf[k].G == Gi) {
        printed = true;
        break;
      }
    ...
    double vmax_speed = 0, vmax_ulpm = 0, vmax_ulpmean = 0, vmax_ulprms = 0,
           vmax_relmax = 0, vmax_relmean = 0, vmax_relrms = 0;

    for (uint32_t j = 0; j < g_rows_uqf_count; ++j) {
      const auto &r = g_rows_uqf[j];
      if (r.op_name != name_i || r.F != Fi || r.G != Gi ||
    ...
    for (int r = 0; r < n; ++r) {
      ...
      // 1 Speed value
      fmt_float(cells[r][1], sizeof(cells[r][1]),
                use_cycles ? row.speed_cycles : row.speed_us,
                use_cycles ? "%0.2f" : "%0.3f");
      // 2 Speed graph
      make_bar(cells[r][2], sizeof(cells[r][2]),
               use_cycles ? row.speed_cycles : row.speed_us, vmax_speed,
               GRAPH_W);

      // ULP max (3,4)
      fmt_uint(cells[r][3], sizeof(cells[r][3]), row.abs_ulp_max);
      make_bar(cells[r][4], sizeof(cells[r][4]), (double)row.abs_ulp_max,
               vmax_ulpm, GRAPH_W);
      // ULP mean (5,6)
      fmt_float(cells[r][5], sizeof(cells[r][5]), row.abs_ulp_mean, "%0.3f");
      make_bar(cells[r][6], sizeof(cells[r][6]), row.abs_ulp_mean, vmax_ulpmean,
               GRAPH_W);
      // ULP rms   (7,8)
      fmt_float(cells[r][7], sizeof(cells[r][7]), row.abs_ulp_rms, "%0.3f");
      make_bar(cells[r][8], sizeof(cells[r][8]), row.abs_ulp_rms, vmax_ulprms,
               GRAPH_W);
      // Rel max   (9,10)
      fmt_uint(cells[r][9], sizeof(cells[r][9]), row.rel_ppm_max);
      make_bar(cells[r][10], sizeof(cells[r][10]), (double)row.rel_ppm_max,
               vmax_relmax, GRAPH_W);
      // Rel mean (11,12)
      fmt_float(cells[r][11], sizeof(cells[r][11]), row.rel_ppm_mean, "%0.3f");
      make_bar(cells[r][12], sizeof(cells[r][12]), row.rel_ppm_mean,
               vmax_relmean, GRAPH_W);
      // Rel rms  (13,14)
      fmt_float(cells[r][13], sizeof(cells[r][13]), row.rel_ppm_rms, "%0.3f");
      make_bar(cells[r][14], sizeof(cells[r][14]), row.rel_ppm_rms, vmax_relrms,
               GRAPH_W);
    }
  ...
  for (uint32_t i = 0; i < g_rows_float_count; ++i) {
    ...
    for (int r = 0; r < n; ++r) {
      ...
      make_bar(cells[r][4], sizeof(cells[r][4]), (double)row.rel_ppm_max,
               vmax_relmax, GRAPH_W);

      fmt_float(cells[r][5], sizeof(cells[r][5]), row.rel_ppm_mean, "%0.3f");
      make_bar(cells[r][6], sizeof(cells[r][6]), row.rel_ppm_mean, vmax_relmean,
               GRAPH_W);

      fmt_float(cells[r][7], sizeof(cells[r][7]), row.rel_ppm_rms, "%0.3f");
      make_bar(cells[r][8], sizeof(cells[r][8]), row.rel_ppm_rms, vmax_relrms,
               GRAPH_W);
    }
