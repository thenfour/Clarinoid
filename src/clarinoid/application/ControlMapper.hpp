...
struct SynthPresetMappableFunction : FunctionHandler {
  ...
  virtual void FunctionHandler_Update(const ControlValue &v) override {
    ...
    if (old != nv) {
      ...
      mInputSrc->InputSource_ShowToast(String("Synth preset: ") + nv + " (" +
                                       (nv - old) + ")\r\n" + p.mName);
...
struct TransposeMappableFunction : FunctionHandler {
  ...
  virtual void FunctionHandler_Update(const ControlValue &v) override {
    ...
    if (old != nv) {
      mInputSrc->InputSource_ShowToast(String("Transpose: ") + nv + " (" +
                                       (nv - old) + ")");
...
struct InputDelegator {
  ...
  FunctionHandler *mHandlers[(size_t)ControlMapping::Function::COUNT] = {
      nullptr};
  ...
  void Init(AppSettings *appSettings, IInputSource *psrc) {
    ...
    RegisterFunction(ControlMapping::Function::ModifierCourse,
                     &mModifierCourse);
    ...
    RegisterFunction(ControlMapping::Function::BaseNoteHoldToggle,
                     &mBaseNoteHoldToggle);
    RegisterFunction(ControlMapping::Function::MetronomeLEDToggle,
                     &mMetronomeLEDToggle);
  ...
  void Update() {
    ...
    for (size_t i = 0; i < SizeofStaticArray(mpAppSettings->mControlMappings);
         ++i) {
      ...
      if (!MatchesModifierKeys(mapping))
        continue;
      if ((size_t)mapping.mFunction >=
          SizeofStaticArray(mHandlers)) // maybe it's just not registered yet?
        continue;

      FunctionHandler *dest =
          mHandlers[(size_t)mapping.mFunction]; // get the function this is
                                                // mapped to
      if (!!dest) {
        auto src = mpSrc->InputSource_GetControl(
            mapping
                .mSource); // and the source control providing the value to map.
  ...
  void RegisterFunction(ControlMapping::Function f, FunctionHandler *handler) {
    ...
    CCASSERT(mHandlers[(size_t)f] ==
             nullptr); // do not register a handler more than once.
