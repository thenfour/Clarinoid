...
AudioConnection mPatchDelayFilterToFeedbackAmpLeft = {delayFilterLeft, 0,
                                                      delayFeedbackAmpLeft, 0};

::clarinoid::FilterNode delayFilterRight;
AudioConnection mPatchDelayToFilterRight = {delayRight, 0, delayFilterRight, 0};
AudioConnection mPatchDelayFilterToFeedbackAmpRight = {
    delayFilterRight, 0, delayFeedbackAmpRight, 0};

// delay output connection
AudioConnection mPatchDelayFilterToWaveMixerLeft = {delayFilterLeft, 0,
                                                    postMixerLeft, 1};
AudioConnection mPatchDelayFilterToWaveMixerRight = {delayFilterRight, 0,
                                                     postMixerRight, 1};

// voice mixer & dry output connection
::clarinoid::MultiMixer<MAX_SYNTH_VOICES>
    voiceMixerDryLeft; // all voices input here.
::clarinoid::MultiMixer<MAX_SYNTH_VOICES>
    voiceMixerDryRight; // all voices input here.
AudioConnection patchVoicesDryToOutpLeft = {voiceMixerDryLeft, 0, postMixerLeft,
                                            0};
AudioConnection patchVoicesDryToOutpRight = {voiceMixerDryRight, 0,
                                             postMixerRight, 0};

// delay input mix
::clarinoid::MultiMixer<MAX_SYNTH_VOICES + 1>
...
AudioConnection patchDelayInputMixToDelayL = {delayInputMixerLeft, 0, delayLeft,
                                              0};
AudioConnection patchDelayInputMixToDelayR = {delayInputMixerRight, 0,
                                              delayRight, 0};

// delay fb
AudioConnection patchDelayFBBackToInputLeft = {
    delayFeedbackAmpLeft, 0, delayInputMixerLeft, MAX_SYNTH_VOICES};
AudioConnection patchDelayFBBackToInputRight = {
    delayFeedbackAmpRight, 0, delayInputMixerRight, MAX_SYNTH_VOICES};

// verb input mix
::clarinoid::MultiMixer<MAX_SYNTH_VOICES + 1>
    verbInputMixerLeft; // all voices input here + 1 for delay line
::clarinoid::MultiMixer<MAX_SYNTH_VOICES + 1>
    verbInputMixerRight; // all voices input here + 1 for delay line
AudioConnection patchVerbInputToVerbL = {verbInputMixerLeft, 0, verbInputMixer,
                                         0};
AudioConnection patchVerbInputToVerbR = {verbInputMixerRight, 0, verbInputMixer,
                                         1};

// delay verb
AudioConnection patchDelayVerbInputLeft = {
    delayFilterLeft, 0, verbInputMixerLeft, MAX_SYNTH_VOICES};
AudioConnection patchDelayVerbInputRight = {
    delayFilterRight, 0, verbInputMixerRight, MAX_SYNTH_VOICES};
...
struct Voice {
  ...
  ::clarinoid::GainAndPanSplitterNode<3>
      mPannerSplitter; // handles panning and send levels to delay/verb/dry
  ...
  void EnsurePatchConnections(AppSettings *appSettings,
                              IModulationSourceSource *modulationSourceSource) {
  ...
  static float CalcFilterCutoffFreq(float breath01, float midiNote,
                                    float keyTrackingAmt, float freqMin,
                                    float freqMax) {
    ...
    float filterKS =
        map(midiNote, 20, 120, 0.0f, 1.0f); // map midi note to full ks effect
  ...
  float GetModulatedValue(float baseOutpVal /* in dest range */,
                          ModulationDestination destType) {
    float ret = baseOutpVal;
    for (const SynthModulationSpec &modSpec : mPreset->mModulations) {
      if (modSpec.mDest != destType)
        continue;
      float srcVal = mModulationSourceSource->GetCurrentModulationSourceValue(
          modSpec.mSource);
  ...
  void Update(const MusicalVoice &mv) {
    ...
    bool voiceOrPatchChanged = (mRunningVoice.mVoiceId != mv.mVoiceId) ||
                               (mRunningVoice.mSynthPatch != mv.mSynthPatch);
    ...
    mOsc.amplitude(1, GetModulatedValue(mPreset->mOsc1Gain,
                                        ModulationDestination::Osc1Volume));
    mOsc.amplitude(2, GetModulatedValue(mPreset->mOsc2Gain,
                                        ModulationDestination::Osc2Volume));
    mOsc.amplitude(3, GetModulatedValue(mPreset->mOsc3Gain,
                                        ModulationDestination::Osc3Volume));

    // update
    float midiNote =
        (float)mv.mMidiNote + mv.mPitchBendN11.GetFloatVal() *
                                  mAppSettings->mSynthSettings.mPitchBendRange;
    ...
    mOsc.pulseWidth(1,
                    GetModulatedValue(mPreset->mOsc1PulseWidth,
                                      ModulationDestination::Osc1PulseWidth));
    mOsc.pulseWidth(2,
                    GetModulatedValue(mPreset->mOsc2PulseWidth,
                                      ModulationDestination::Osc2PulseWidth));
    mOsc.pulseWidth(3,
                    GetModulatedValue(mPreset->mOsc3PulseWidth,
                                      ModulationDestination::Osc3PulseWidth));

    mOsc.frequency(1,
                   MIDINoteToFreq(midiNote + mPreset->mOsc1PitchFine +
                                  mPreset->mOsc1PitchSemis - mPreset->mDetune));
    mOsc.frequency(3,
                   MIDINoteToFreq(midiNote + mPreset->mOsc3PitchFine +
                                  mPreset->mOsc3PitchSemis + mPreset->mDetune));

    if (mPreset->mSync) {
      float freq = MIDINoteToFreq(midiNote + mPreset->mOsc2PitchFine +
                                  mPreset->mOsc2PitchSemis);
      float freqSync =
          map(mv.mBreath01.GetFloatVal(), 0.0f, 1.0f,
              freq * mPreset->mSyncMultMin, freq * mPreset->mSyncMultMax);
      mOsc.frequency(2, freqSync);
    } else {
      mOsc.frequency(2, MIDINoteToFreq(midiNote + mPreset->mOsc2PitchFine +
                                       mPreset->mOsc2PitchSemis));
    }
    ...
    float filterFreq = CalcFilterCutoffFreq(
        mv.mBreath01.GetFloatVal(), midiNote, mPreset->mFilterKeytracking,
        mPreset->mFilterMinFreq, mPreset->mFilterMaxFreq);

    mFilter.SetParams(mPreset->mFilterType, filterFreq, mPreset->mFilterQ,
                      mPreset->mFilterSaturation);
    ...
    mPannerSplitter.SetPanAndGain(1, mPreset->mDelaySend,
                                  mv.mPan + mPreset->mPan);
    mPannerSplitter.SetPanAndGain(2, mPreset->mVerbSend,
                                  mv.mPan + mPreset->mPan);
  ...
  void Unassign() { mRunningVoice.mVoiceId = MAGIC_VOICE_ID_UNASSIGNED; }

  Voice(int16_t vid)
      : mPatchOutDryLeft(mPannerSplitter, 0, CCSynthGraph::voiceMixerDryLeft,
                         vid),
        mPatchOutDryRight(mPannerSplitter, 1, CCSynthGraph::voiceMixerDryRight,
                          vid),
        mPatchOutDelayLeft(mPannerSplitter, 2,
                           CCSynthGraph::delayInputMixerLeft, vid),
        mPatchOutDelayRight(mPannerSplitter, 3,
                            CCSynthGraph::delayInputMixerRight, vid),
        mPatchOutVerbLeft(mPannerSplitter, 4, CCSynthGraph::verbInputMixerLeft,
                          vid),
        mPatchOutVerbRight(mPannerSplitter, 5, CCSynthGraph::verbInputMixerLeft,
                           vid) {}
};
...
struct SynthGraphControl {
  ...
  void Setup(AppSettings *appSettings, Metronome *metronome,
    ...
    AudioStream::initialize_memory(CLARINOID_AUDIO_MEMORY,
                                   SizeofStaticArray(CLARINOID_AUDIO_MEMORY));
  ...
  void EndUpdate() { AudioInterrupts(); }

  void UpdatePostFx() {
    CCSynthGraph::delayFeedbackAmpLeft.gain(
        mAppSettings->mSynthSettings.mDelayFeedbackLevel);
    CCSynthGraph::delayFeedbackAmpRight.gain(
        mAppSettings->mSynthSettings.mDelayFeedbackLevel);
    CCSynthGraph::delayLeft.delay(0, mAppSettings->mSynthSettings.mDelayMS);
    CCSynthGraph::delayRight.delay(
        0, mAppSettings->mSynthSettings.mDelayMS +
               mAppSettings->mSynthSettings.mDelayStereoSep);

    CCSynthGraph::delayFilterLeft.SetParams(
        mAppSettings->mSynthSettings.mDelayFilterType,
    ...
    CCSynthGraph::verbWetAmpRight.gain(
        mAppSettings->mSynthSettings.mReverbGain);
    ...
    short wantsWaveType1 =
        convertWaveType(mAppSettings->mSynthSettings.mLfo1Shape);
    short wantsWaveType2 =
        convertWaveType(mAppSettings->mSynthSettings.mLfo2Shape);
    ...
    if (!mAppSettings->mMetronomeSoundOn) {
      ...
      CCSynthGraph::metronomeOsc.frequency(
          MIDINoteToFreq(mAppSettings->mMetronomeNote));

      float metronomeBeatFrac = mMetronome->GetBeatFrac();
      if (metronomeBeatFrac <
          mPrevMetronomeBeatFrac) { // beat boundary is when the frac drops back
                                    // to 0
