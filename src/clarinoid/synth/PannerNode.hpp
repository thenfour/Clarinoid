...
inline static void
audioBufferCopyAndApplyGainTwice(int16_t *inp, int16_t *outp1, int32_t mult1,
                                 int16_t *outp2, int32_t mult2) {
...
struct PannerNode : public AudioStream {
  PannerNode() : AudioStream(1, inputQueueArray) { SetPan(0); }
  ...
  virtual void update() override {
    ...
    audioBufferCopyAndApplyGainTwice(inputBuf->data, outputBufLeft->data,
                                     mMultiplierLeft, outputBufRight->data,
                                     mMultiplierRight);
...
struct GainAndPanSplitterNode : public AudioStream {
  ...
  void SetPanAndGain(size_t chan, float gain01, float panN11) {
    ...
    if (gain01 < 0)
      gain01 = 0;
    if (FloatEquals(panN11, mPanN11[chan], 0.00001f) &&
        FloatEquals(gain01, mGain01[chan], 0.00001f)) {
      return;
    }
  ...
  virtual void update() override {
    ...
    audioBufferCopyAndApplyGainMulti<OutputBufferCount>(
        inputBuf->data, outputDataBuffers, mMultipliers);
