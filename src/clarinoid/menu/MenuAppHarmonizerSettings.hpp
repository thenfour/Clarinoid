...
struct HarmVoiceSettingsApp
{
  ...
  HarmVoiceSettingsApp(IAppDisplay& d)
    : mDisplay(d)
  {
  }
  ...
  EnumSettingItem<ScaleFlavorIndex> mLocalScaleFlavor = { " ->Flav",
  ...
  EnumSettingItem<NonDiatonicBehavior> mNonDiatonicBehavior = { "OOS behav",
  ...
  EnumSettingItem<PitchBendParticipation> mPitchbendBehav = { "PB behav",
  ...
  ISettingItem* mArray[12] = {
    &mSynthPresetRef, &mOwnSynthPatch,  &mSequenceLength, &mSequence,       &mMinOutpNote,      &mMaxOutpNote,
    &mOOBBehavior,    &mPitchbendBehav, &mScaleRefType,   &mLocalScaleNote, &mLocalScaleFlavor, &mNonDiatonicBehavior,
  };
...
{
  virtual const char* DisplayAppGetName() override { return "HarmPatchSettingsApp"; }

  HarmPatchSettingsApp(IAppDisplay& d)
    : SettingsMenuApp(d)
    , mDisplay(d)
    , mHarmVoiceSettingsApp(d)
  {
  }
  ...
  EnumSettingItem<ScaleFlavorIndex> mPresetScaleFlavor = { " ->Flav",
                                                           gScaleFlavorIndexInfo,
                                                           Property<ScaleFlavorIndex>{
                                                             [](void* cap) FLASHMEM {
                                                               auto* pThis = (HarmPatchSettingsApp*)cap;
  ...
  FunctionListSettingItem mCopyPreset = { "Copy to ...",
                                          ...
                                          [](void* cap, size_t i) { // itemNameGetter,
                                            ...
                                            return String(String("") + i + ":" +
                                                          pThis->mAppSettings->mHarmSettings.mPresets[i].mName);
                                          },
                                          [](void* cap,
  ...
  ISettingItem* mArray[11] = {
    &mEmitLiveNote, &mStereoSeparation, &mMinRotationTimeMS, &mPresetScaleNote, &mPresetScaleFlavor, &mSynthPreset1,
    &mSynthPreset2, &mSynthPreset3,     &mSynthPreset4,      &mVoiceSubmenu,    &mCopyPreset,
  };
  SettingsList mRootList = { mArray };
...
struct HarmSettingsApp : public SettingsMenuApp
{
  ...
  HarmSettingsApp(IAppDisplay& d)
    : SettingsMenuApp(d)
    , mHarmPatchSettings(d)
  {
  }
  ...
  virtual void RenderFrontPage()
  {
    ...
    this->mDisplay.PrintLine("Harmonizer >");
