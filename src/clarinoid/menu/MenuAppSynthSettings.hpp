...
struct EnvelopeMenuApp
{
  ...
  ISettingItem* mArray[7] = {
    &mDelayMS, &mAttackMS, &mHoldMS, &mDecayMS, &mSustainLevel, &mReleaseMS, &mReleaseNoteOnMS,
  };
...
struct SynthPatchOscillatorMenuStuff
{
  ...
  SynthOscillatorSettings& GetBinding() { return *mpBinding; }
...
struct SynthPatchMenuApp : public SettingsMenuApp
{
  ...
  SynthPatchMenuApp(IAppDisplay& d)
    : SettingsMenuApp(d) //, mDisplay(d)
  {
  }

  // int16_t mBindingID = 0;
  SynthPreset& GetBinding() { return GetAppSettings()->FindSynthPreset(GetBindingID()); }
  ...
  // EnumSettingItem(const String& name, const EnumInfo<T>& enumInfo, const Property<T>& binding,
  // cc::function<bool()>::ptr_t isEnabled) :
  EnumSettingItem<ClarinoidFilterType> mBreathFiltType = { "Filter",
  ...
  FunctionListSettingItem mCopyPreset = { "Copy to ...",
                                          SYNTH_PRESET_COUNT,
                                          ...
                                          [](void* cap, size_t i) { // cc::function<void(void*,size_t)>::ptr_t onClick,
                                            ...
                                            auto fromName =
                                              pThis->GetAppSettings()->GetSynthPatchName(pThis->GetBindingID());
  ...
  SynthModulationSpec& GetModulationBinding() { return GetBinding().mModulations[mEditingModulationIndex]; }

  EnumSettingItem<AnyModulationSource> mModSource = { "Source",
                                                      gAnyModulationSourceInfo,
                                                      Property<AnyModulationSource>{
                                                        [](void* cap) FLASHMEM {
                                                          auto* pThis = (SynthPatchMenuApp*)cap;
  ...
  EnumSettingItem<AnyModulationDestination> mModDest = { "Dest",
                                                         gAnyModulationDestinationInfo,
                                                         Property<AnyModulationDestination>{
                                                           [](void* cap) FLASHMEM {
                                                             auto* pThis = (SynthPatchMenuApp*)cap;
  ...
  EnumSettingItem<AnyModulationSource> mModAuxSource = { " - Aux source",
                                                         gAnyModulationSourceInfo,
                                                         Property<AnyModulationSource>{
                                                           [](void* cap) FLASHMEM {
                                                             auto* pThis = (SynthPatchMenuApp*)cap;
  ...
  ISettingItem* mModulationSubmenu[10] = {
    &mModSource,    &mModDest,      &mModScale,     &mModSourcePolarity, &mModCurve,

    &mModAuxEnable, &mModAuxSource, &mModAuxAmount, &mModAuxPolarity,    &mModAuxCurve,
  };
  ...
  ISettingItem* mBreathFilterSubmenuArray[6] = {
    &mBreathFiltType, &mBreathFiltMin, &mBreathFiltMax, &mBreathFiltKS, &mBreathFiltQ, &mBreathFiltSaturation,
  };
  ...
  {
    mDisplay.ClearState();
    mDisplay.PrintLine(String("Synth Patch >"));
    mDisplay.PrintLine(GetAppSettings()->GetSynthPatchName(GetBindingID()));
