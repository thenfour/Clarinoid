...
struct FunctionListEditor : ISettingItemEditor
{
  ...
  FunctionListEditor(size_t itemCount, cc::function<String(void*,size_t)>::ptr_t itemNameGetter, cc::function<void(void*,size_t)>::ptr_t onClick, void* cap) :
    mOnClick(onClick),
    mCap(cap)
  {
    mList.mCap = cap;
  ...
  virtual void SetupEditing(ISettingItemEditorActions* papi, int x, int y) {
    CCASSERT(!!papi);
    mpApi = papi;
    mListControl.Init(&mList, &mpApi->GetInputDelegator()->mMenuScrollA, Property<int> {
      [](void* cap) {
...
struct FunctionListSettingItem : public ISettingItem
{
  ...
  FunctionListSettingItem(const String& name, size_t itemCount, cc::function<String(void*,size_t)>::ptr_t itemNameGetter, cc::function<void(void*,size_t)>::ptr_t onClick, const Property<bool>& isEnabled, void* cap) :
    mName(name),
    mEditor(itemCount, itemNameGetter, onClick, cap),
