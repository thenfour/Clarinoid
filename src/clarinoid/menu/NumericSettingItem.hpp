...
struct NumericEditor : ISettingItemEditor
{
    ...
    NumericEditor(const NumericEditRangeSpec<T> &range_, const Property<T> &binding) : mRange(range_), mBinding(binding)
    {
    }
...
struct MultiNumericSettingItem : public ISettingItem
{
    ...
    MultiNumericSettingItem(cc::function<size_t(void *)>::ptr_t getItemCount,
                            const NumericEditRangeSpec<T> &range_,
        ...
        :
          ...
          mGetItemCount(getItemCount), mLabelGetter(labelGetter), mValueStringGetter(valueStringGetter),
    ...
    virtual String GetName(size_t multiIndex)
    {
        return mLabelGetter(mCapture, multiIndex);
    }
    virtual String GetValueString(size_t multiIndex)
    {
        return mValueStringGetter(mCapture, multiIndex);
    }
    virtual SettingItemType GetType(size_t multiIndex)
    {
        return SettingItemType::Custom;
