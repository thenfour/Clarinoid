...
struct ISettingItem
{
    ...
    {
        return 1;
    } // return 1 if this is 1 item. otherwise this item gets replicated N times, with an index parameter.

    virtual String GetName(size_t multiIndex) = 0;
    virtual String GetValueString(size_t multiIndex)
    {
        return "";
    } // not used for trigger types or submenu types
    ...
    virtual struct ISettingItemEditor *GetEditor(size_t multiIndex)
    {
        return nullptr;
    } // for custom types
...
struct SubmenuSettingItem : public ISettingItem
{
    ...
    SubmenuSettingItem(const String &name,
    ...
    virtual SettingItemType GetType(size_t multiIndex)
    {
        return SettingItemType::Submenu;
    }
    virtual bool IsEnabled(size_t multiIndex) const
    {
...
struct MultiSubmenuSettingItem : public ISettingItem
{
    ...
    MultiSubmenuSettingItem(typename cc::function<size_t(void *)>::ptr_t getItemCount,
                            typename cc::function<String(void *, size_t)>::ptr_t name,
                            typename cc::function<SettingsList *(void *, size_t)>::ptr_t pGetSubmenu,
                            typename cc::function<bool(void *, size_t)>::ptr_t isEnabled,
                            void *capture)
        : mGetItemCount(getItemCount), mGetSubmenu(pGetSubmenu), mName(name), mIsEnabled(isEnabled), mpCapture(capture)
    {
    }

    void Init(typename cc::function<size_t(void *)>::ptr_t getItemCount,
              typename cc::function<String(void *, size_t)>::ptr_t name,
              typename cc::function<SettingsList *(void *, size_t)>::ptr_t pGetSubmenu,
              typename cc::function<bool(void *, size_t)>::ptr_t isEnabled,
              void *capture)
    ...
    virtual String GetName(size_t multiIndex)
    {
        return mName(mpCapture, multiIndex);
    }
    virtual SettingItemType GetType(size_t multiIndex)
    {
        return SettingItemType::Submenu;
...
struct LabelSettingItem : public ISettingItem
{
    ...
    LabelSettingItem(const Property<String> &text, const Property<bool> &isEnabled) : mText(text), mIsEnabled(isEnabled)
    {
    }
...
struct MultiLabelSettingItem : public ISettingItem
{
    ...
    MultiLabelSettingItem(cc::function<size_t(void *)>::ptr_t getItemCount,
                          cc::function<String(void *, size_t)>::ptr_t text,
                          cc::function<bool(void *, size_t)>::ptr_t isEnabled,
                          void *capture)
        : mGetItemCount(getItemCount), mText(text), mIsEnabled(isEnabled), mCapture(capture)
    {
    }

    virtual size_t GetMultiCount()
    {
        return mGetItemCount(mCapture);
    }
    virtual String GetName(size_t multiIndex)
    {
        return mText(mCapture, multiIndex);
    }
    virtual String GetValueString(size_t multiIndex)
    {
        return "";
...
struct StringSettingItem : public ISettingItem
{
    ...
    StringSettingItem(cc::function<String()>::ptr_t text, cc::function<bool()>::ptr_t isEnabled)
        : mText(text), mIsEnabled(isEnabled)
    {
    }
...
struct SettingsMenuApp : DisplayApp, ISettingItemEditorActions
{
    ...
    virtual InputDelegator *GetInputDelegator() override
    {
        return mInput;
    }
