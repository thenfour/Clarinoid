...
struct ISettingItemEditorActions
{
  ...
  virtual IAppDisplay* GetDisplay() = 0;
...
struct TriggerSettingItem : public ISettingItem
{
  ...
  virtual void Trigger(size_t multiIndex) { mAction(mActionCapture); }
};
...
struct MultiSubmenuSettingItem : public ISettingItem
{
  ...
  virtual struct SettingsList* GetSubmenu(size_t multiIndex) { return mGetSubmenu(mpCapture, multiIndex); }
};
...
struct SettingsMenuApp : DisplayApp, ISettingItemEditorActions
{
  ...
  SettingsMenuApp(IAppDisplay& d)
    : DisplayApp(d)
  {
  }

  virtual void CommitEditing() override { mpCurrentEditor = nullptr; }
  ...
  virtual IAppDisplay* GetDisplay() override { return &mDisplay; }
  ...
  virtual void RenderApp()
  {
    ...
    for (size_t i = 0; i < itemsToRender; ++i) {
      ...
      auto cursorY = mDisplay.GetCursor().y;
      ...
      if (itemToRender == (state.pList->Count() - 1)) {
        ...
        mDisplay.DrawHLine(0, separatorY, mDisplay.ScreenSize().width, SSD1306_INVERSE);
      }
