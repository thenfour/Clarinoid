...
struct SystemSettingsApp : SettingsMenuApp
{
  ...
  SystemSettingsApp(IAppDisplay& d,
  ...
  UnipolarCalibrationSettingItem mBreath = {
    "Breath",
    Property<UnipolarMapping>{ [](void* cap) FLASHMEM {
                                auto pThis = (SystemSettingsApp*)cap;
                                UnipolarMapping ret =
  ...
  ISettingItem* mArray[7] = {
    &mSelectedPerfPatch, &mDimDisplay, &mBreath, &mPitchUp, &mPitchDown, &mNoteChangeFrames, &mNoteChangeIntFrames,
  };
...
public:
  ...
  {
    mDisplay.ClearState();
    mDisplay.PrintLine(String("System >"));
