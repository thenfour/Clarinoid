...
struct ListControl
{
  ...
  void Render(IAppDisplay* mDisplay, int x, int y, int mVisibleItems)
  {
    ...
    for (int i = 0; i < itemsToRender; ++i) {
      mDisplay->PrintInvertedLine(mpList->List_GetItemCaption(itemToRender), itemToRender == mSelectedItem.GetValue());
...
struct ListControl2
{
  ...
  void Render(IAppDisplay* mDisplay,
              const RectI& rc,
              int itemCount,
              ...
              void* capture)
  {
    ...
    mDisplay->SetClipRect(rc);
    mDisplay->SetCursor(rc.TopLeft());
    mDisplay->SetTextLeftMargin(rc.Left());
    for (int i = 0; i < itemsToRender; ++i) {
      ...
      if (itemToRender == (itemCount - 1)) {
        ...
        if (rc.YInRect(separatorY)) {
          mDisplay->DrawHLine(rc.Left(), separatorY, rc.Width(), SSD1306_INVERSE);
        }
      }
    }
  }
