...
//   constexpr void cacheDouble() { mDoubleValue = ToDouble(); }
// #else
...
// private:
//   explicit constexpr Fixed(BaseType v)
//     : mValue(v)
//   {
//     cacheDouble();
...
// public:
//   static constexpr MyT FromFixed(BaseType v) { return MyT{ v }; }
...
//   constexpr Fixed(const T& v)
//     : mValue(static_cast<BaseType>(v * mScalingValue))
//   {
//     cacheDouble();
//     performRuntimeChecks();
//   }
...
//   constexpr double ToDouble() const { return (double)mValue / mScalingValue; }

//   constexpr float ToFloat() const { return (float)mValue / mScalingValue; }
...
//     // uint64 => int64.
//     // anything else => int32
//     using SignedBaseType = typename std::
//       conditional<std::is_same<uint64_t, typename std::make_unsigned<BaseType>::type>::value, int64_t,
//       int32_t>::type;
...
//     static constexpr uint8_t kShiftNeeded = std::max(0, kOriginalWidth - kNewMaxWidth); // this will be either 0 or 1
...
//   using CorrespondingSignedType =
//     typename std::conditional<std::is_signed<BaseType>::value, MyT, typename
//     UnsignedToSignedHelper::SignedType>::type;

// public:
//   constexpr CL_NODISCARD CorrespondingSignedType MakeSigned() const { return *this; }
...
//     using ResultBaseType =
//       typename std::conditional<ResultAllows64Bit, FPAutoBaseType3264<IdealWidth>,
//       FPAutoBaseType32<IdealWidth>>::type;
...
//   constexpr CL_NODISCARD bool IsApproximatelyEqualTo(const Fixed<TIntBitsB, TFractBitsB, BaseTypeB>& b, T epsilon)
//   const
//   {
//     auto t1 = this->Subtract(b);
//     auto t2 = t1.Abs();
...
//       x1
//     }; // convert to Q16 to ease calculations below (otherwise we're dipping into int64 territory)
...
//   typename BaseTypeC,
//   typename TypeHelper =
//     typename Fixed<TIntBitsA, TFractBitsA, BaseTypeA>::template DifferenceTypeHelper<TIntBitsB, TFractBitsB,
//     BaseTypeB>,
//   typename ResultType = typename TypeHelper::ResultType>
// ResultType
// Lerp(const Fixed<TIntBitsA, TFractBitsA, BaseTypeA>& a,
...
// ResultType
// Lerp(const Fixed<TIntBitsA, TFractBitsA, BaseTypeA>& a, const Fixed<TIntBitsB, TFractBitsB, BaseTypeB>& b, T t01)
// {
...
// static inline constexpr Fixed<StaticValueBitsNeeded<value>::value, fractBits, int32_t>
// FixedInteger()
// {
//   return Fixed<StaticValueBitsNeeded<value>::value, fractBits, int32_t>::FromValue(value);
// }

// static inline Fixed<0, 16, uint16_t>
// sqrt(const Fixed<0, 32, uint32_t>& x)
// {
//   using ReturnType = Fixed<0, 16, uint16_t>;
