...
static size_t UnifyCircularBuffer_Left(const Ptr &segmentABegin,
                                       const Ptr &segmentAEnd,
                                       const Ptr &segmentBBegin,
                                       const Ptr &segmentBEnd,
                                       uint8_t (&tempBuffer)[tempBufferBytes])
{
    ...
    if (sizeA >= sizeB)
    {
        ...
        //      |Bbb--aaaaaa|
        // =>   |aaaaaaBbb--|
        UnifyCircularBuffer_Left(segmentABegin.PlusBytes(len),
                                 segmentAEnd,
                                 segmentBBegin.PlusBytes(len),
                                 segmentBBegin.PlusBytes(len + sizeB),
                                 tempBuffer);
...
static size_t UnifyCircularBuffer_Left(void *segmentABegin,
                                       void *segmentAEnd,
                                       void *segmentBBegin,
                                       void *segmentBEnd,
                                       uint8_t (&tempBuffer)[tempBufferBytes])
{
    return UnifyCircularBuffer_Left(
        Ptr(segmentABegin), Ptr(segmentAEnd), Ptr(segmentBBegin), Ptr(segmentBEnd), tempBuffer);
}
