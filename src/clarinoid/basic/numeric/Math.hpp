...
void
arm_q15_to_float(const int16_t* in, float* out, size_t n)
{
...
void
arm_fill_f32(float val, float* out, size_t n)
{
  for (size_t i = 0; i < n; ++i) {
    out[i] = val;
  }
...
void
arm_fill_q15(int16_t val, int16_t* out, size_t n)
{
  for (size_t i = 0; i < n; ++i) {
    out[i] = val;
  }
...
void
arm_add_q15(int16_t* pSrcA, int16_t* pSrcB, int16_t* pDst, uint32_t blockSize)
{
...
void
arm_offset_q15(int16_t* pSrc, int16_t offset, int16_t* pDst, uint32_t blockSize)
{
  for (uint32_t i = 0; i < blockSize; ++i) {
...
void
arm_scale_q15(int16_t* pSrc, int16_t scaleFract, int8_t shift, int16_t* pDst, uint32_t blockSize)
{
  for (uint32_t i = 0; i < blockSize; ++i) {
...
pack_16b_16b(int32_t a, int32_t b)
{
...
static inline uint32_t
signed_add_16_and_16(uint32_t a, uint32_t b)
{
  union packed
  {
    uint32_t u32;
...
signed_multiply_32x16b(int32_t a, uint32_t b)
{
...
static inline int32_t
signed_multiply_32x16t(int32_t a, uint32_t b)
{
...
inline void
Sample32To16Buffer(const float* in, int16_t* out)
{
...
// template <size_t N>
inline void
Sample16To32Buffer(int16_t* in, float* out)
{
...
inline void
FillBufferWithConstant(float val, float* out)
{
...
// 20x faster than naive copy
inline void
FillBufferWithConstant(int16_t val, int16_t* out)
{
...
inline void
BufferScaleInPlace(float* buf, float scale)
{
...
// https://arm-software.github.io/CMSIS_5/DSP/html/group__BasicScale.html#ga5e769c8e22da173674c6abca7291e713
inline void
BufferOffsetInPlace(float* buf, float offset)
{
...
uint16_t
ClampUint32ToUint16(uint32_t a)
{
...
inline bool
FloatEquals(float f1, float f2, float eps = FloatEpsilon)
{
  return fabs(f1 - f2) < eps;
}

inline bool
FloatLessThanOrEquals(float lhs, float rhs, float eps = FloatEpsilon)
{
  return lhs <= (rhs + eps);
}

inline bool
FloatIsAbove(float lhs, float rhs, float eps = FloatEpsilon)
{
  return lhs > (rhs + eps);
}
...
inline bool
NumberEquals(float lhs, float rhs)
{
  return FloatEquals(lhs, rhs);
}
inline bool
FloatRoundedEqualsInt(float f, int n)
{
  return (int)FloatRoundToInt(f) == n;
}

static float
Clamp(float x, float low, float hi)
{
  if (x <= low)
    return low;
  if (x >= hi)
...
template<typename T>
static T
ClampInclusive(T x, T minInclusive, T maxInclusive)
{
  if (x <= minInclusive)
    return minInclusive;
  if (x >= maxInclusive)
...
static float
RemapTo01(float x, float xmin, float xmax)
{
...
// remap so src min to max become [0-1]. results are NOT clamped.
static float
RemapTo01Clamped(float x, float xmin, float xmax)
{
  if (FloatEquals(xmax - xmin, 0.0f))
    return xmin;
  x -= xmin;
...
static float
Remap01ToRange(float x01, float destMin, float destMax)
{
...
static float
RemapToRange(float x, float amin, float amax, float bmin, float bmax)
{
...
// 2 hz = 500 ms
float
HertzToCycleMS(float hz)
{
  return 1000.0f / hz;
}

float
CycleMSToHertz(float ms)
{
  return 1000.0f / ms;
}
...
// 1/1 @ 60bpm = 1000 ms.
float
NoteLengthToMS(float bpmIfNeeded, float numerator, float denom)
{
 ...
 */
inline float
LinearToDecibels(float aLinearValue, float aMinDecibels = MIN_DECIBEL_GAIN)
{
 ...
 */
inline float
DecibelsToLinear(float aDecibels, float aNegInfDecibels = MIN_DECIBEL_GAIN)
{
...
// decibels glyph
inline String
DecibelsToIntString(float aDecibels, float aNegInfDecibels = MIN_DECIBEL_GAIN)
{
...
namespace Curve2 {
static float
Curve2_F(float c, float x, float n)
{
...
// returns 0,1 clamped.
static float
Eval(float _x, float _p, float slope)
{
  if (_x <= 0)
    return 0;
  if (_x >= 1)
...
struct UnipolarMapping
{
  ...
  bool IsSrcInRegion(float src) const { return (src >= mSrcMin) && (src <= mSrcMax); }
...
void
DivRem(T val, T& wholeParts, T& remainder)
{
...
void
DivRemBitwise(Tval val, size_t& wholeParts, Tremainder& remainder)
{
...
static int
RotateIntoRange(const int& val, const int& itemCount)
{
...
static uint8_t
RotateIntoRangeByte(int8_t val, uint8_t itemCount)
{
  CCASSERT(itemCount > 0);
  while (val < 0) {
...
static uint8_t
RotateIntoRangeByte(int8_t val, uint8_t itemCount, int8_t& correction)
{
  CCASSERT(itemCount > 0);
  correction = 0;
  while (val < 0) {
...
static inline int
AddConstrained(int orig, int delta, int min_, int max_)
{
  CCASSERT(max_ >= min_);
  if (max_ <= min_)
    return min_;
...
template<typename T>
T
idiv_round(T dividend, T divisor)
{
...
int
ModularDistance(int period, int a, int b)
{
  a = RotateIntoRange(a, period);
  b = RotateIntoRange(b, period);
  if (a > b) {
...
struct SawWave
{
  ...
  void SetFrequency(float f) { mFrequencyMicros = (uint32_t)(1000000.0f / f); }
...
struct PulseWave
{
  ...
  void SetFrequency(float f) { SetFrequencyAndDutyCycle01(f, mDutyCycle01); }

  void SetDutyCycle01(float dc01) { SetFrequencyAndDutyCycle01(mFrequency, dc01); }

  PulseWave() { SetFrequencyAndDutyCycle01(1.0f, 0.5f); }
...
template<typename T>
void
drawLine(int x0, int y0, int x1, int y1, T&& drawPixel)
{
  int dx = abs(x1 - x0);
  int sx = x0 < x1 ? 1 : -1;
  int dy = -abs(y1 - y0);
