...
{
    GenericEnumItemInfo(int intval, const char *name) : mName(name), mIntValue(intval)
    {
    }
    const char *mName;
    int mIntValue;
...
{
    EnumItemInfo(T val, const char *name) : GenericEnumItemInfo((int)val, name), mValue(val)
    {
        //
    }
    T mValue;
...
struct GenericEnumInfo : IList
{
    ...
    GenericEnumInfo(size_t count, const char *name) : mItemCount(count), mTypeName(name)
    {
    }
