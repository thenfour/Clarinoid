...
{
  GenericEnumItemInfo(int intval, const char *name) :
    mName(name),
    mIntValue(intval)
  {
  }
...
{
  EnumItemInfo(T val, const char *name) :
    GenericEnumItemInfo((int)val, name),
    mValue(val)
  {
    //
  }
...
struct GenericEnumInfo : IList
{
  ...
  GenericEnumInfo(size_t count, const char *name) :
    mItemCount(count),
    mTypeName(name)
  {
  }
...
template<typename T>
struct EnumInfo : GenericEnumItemInfo {
  ...
  //virtual const GenericEnumItemInfo* GenericGetItem(size_t n) const override { return GetItem(n); };
