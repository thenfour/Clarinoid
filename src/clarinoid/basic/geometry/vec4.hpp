...
struct Vec4
{
  ...
  // ----------------- element access -----------------
  constexpr T& operator[](std::size_t i) noexcept { return i == 0 ? x : (i == 1 ? y : (i == 2 ? z : w)); }
  constexpr const T& operator[](std::size_t i) const noexcept { return i == 0 ? x : (i == 1 ? y : (i == 2 ? z : w)); }
  ...
  struct elem_iter
  {
    ...
    reference operator*() const { return i == 0 ? v->x : (i == 1 ? v->y : (i == 2 ? v->z : v->w)); }
    elem_iter& operator++()
    {
      ++i;
      return *this;
    }
    bool operator==(const elem_iter& o) const { return v == o.v && i == o.i; }
    bool operator!=(const elem_iter& o) const { return !(*this == o); }
  };
  using iterator = elem_iter<false>;
  using const_iterator = elem_iter<true>;
  iterator begin() noexcept { return { this, 0 }; }
  iterator end() noexcept { return { this, 4 }; }
  const_iterator begin() const noexcept { return { this, 0 }; }
  const_iterator end() const noexcept { return { this, 4 }; }
  const_iterator cbegin() const noexcept { return { this, 0 }; }
  const_iterator cend() const noexcept { return { this, 4 }; }
  ...
  [[nodiscard]] constexpr T dot(const Vec4& v) const noexcept { return x * v.x + y * v.y + z * v.z + w * v.w; }

  [[nodiscard]] constexpr T length2() const noexcept { return x * x + y * y + z * z + w * w; }
  ...
  [[nodiscard]] Vec4 reject_from(const Vec4& v) const noexcept { return *this - project_onto(v); }
  ...
  [[nodiscard]] Vec4 rcp() const noexcept { return { T{ 1 } / x, T{ 1 } / y, T{ 1 } / z, T{ 1 } / w }; }
  ...
  // ----------------- interpolation -----------------
  [[nodiscard]] static constexpr Vec4 lerp(const Vec4& a, const Vec4& b, T t) noexcept { return a + (b - a) * t; }
  ...
  // ----------------- conversions & helpers -----------------
  [[nodiscard]] constexpr std::array<T, 4> to_array() const noexcept { return { x, y, z, w }; }
  ...
  [[nodiscard]] static constexpr Vec4 unit_x() noexcept { return { T{ 1 }, T{ 0 }, T{ 0 }, T{ 0 } }; }
  [[nodiscard]] static constexpr Vec4 unit_y() noexcept { return { T{ 0 }, T{ 1 }, T{ 0 }, T{ 0 } }; }
  [[nodiscard]] static constexpr Vec4 unit_z() noexcept { return { T{ 0 }, T{ 0 }, T{ 1 }, T{ 0 } }; }
  [[nodiscard]] static constexpr Vec4 unit_w() noexcept { return { T{ 0 }, T{ 0 }, T{ 0 }, T{ 1 } }; }
};
