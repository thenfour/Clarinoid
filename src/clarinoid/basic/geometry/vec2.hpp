...
struct Vec2
{
  ...
  // ----------------- element access -----------------
  constexpr T& operator[](std::size_t i) noexcept { return (i == 0) ? x : y; }
  constexpr const T& operator[](std::size_t i) const noexcept { return (i == 0) ? x : y; }
  ...
  Vec2& rotate(T radians) noexcept { return (*this = this->rotated(radians)); }

  // 90-degree left/right perpendiculars
  [[nodiscard]] constexpr Vec2 perp_ccw() const noexcept { return { -y, x }; } // +90°
  [[nodiscard]] constexpr Vec2 perp_cw() const noexcept { return { y, -x }; }  // -90°
  ...
  [[nodiscard]] Vec2 reject_from(const Vec2& v) const noexcept { return *this - project_onto(v); }
  ...
  [[nodiscard]] static Vec2 from_angle(T radians) noexcept { return from_polar(T{ 1 }, radians); }
  ...
  [[nodiscard]] constexpr std::array<T, 2> to_array() const noexcept { return { x, y }; }
