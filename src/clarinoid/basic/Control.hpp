...
struct ISwitch
{
  virtual bool CurrentValue() const { CCASSERT(false); return false; }
};
...
public:
    ...
    explicit SwitchControlReader(ISwitch* c) :
        mC(c)
    {
...
struct IEncoder
{
  virtual EncoderState CurrentValue() const { CCASSERT(false); return {}; }
};
...
public:
    explicit EncoderReader(IEncoder* enc) :
        mC(enc)
    {
    }
    EncoderReader()
