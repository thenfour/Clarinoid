...
struct GuiPerformanceApp : GuiApp
{
  ...
  virtual const char* DisplayAppGetName() override { return "GuiPerformanceApp"; }

  GuiPerformanceApp(IAppDisplay& display, Metronome& m /* PROMISE ME YOU WILL SET mBreathCalibration */)
    : GuiApp(display)
    , mMetronome(m)
  {
  }
  ...
  GuiTransposeControl<int8_t> mTranspose = { 0,
                                             ...
                                             { [](void* cap) -> bool { return true; }, // double-click getter (not used)
                                               [](void* cap,
                                                  const bool& val) { // double-click setter (set transpose to 0)
                                                 auto* pThis = (GuiPerformanceApp*)cap;
  ...
  GuiLabelControl mGlobalScale = { 0,
