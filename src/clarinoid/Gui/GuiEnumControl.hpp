...
struct GuiEnumAsTextRenderer : IGuiRenderer<T>
{
  ...
  GuiEnumAsTextRenderer(const EnumInfo<T>& enumInfo)
    : mEnumInfo(enumInfo)
  {
  }
...
struct GuiEnumEditor : IGuiEditor<T>
{
  ...
  GuiEnumEditor(const EnumInfo<T>& enumInfo, const Property<T>& binding)
    : mBinding(binding)
    , mEnumInfo(enumInfo)
  {
  }
                                 ...
                                 DisplayApp& app) override
  {
    ...
    int nitems = app.mDisplay.GetClipRect().Height() / app.mDisplay.GetLineHeight();
