...
struct GuiApp : public DisplayApp
{
  ...
  virtual void RenderFrontPage() override
  {
    ...
    for (size_t i = 0; i < list->Count(); ++i) {
      ...
      if (ctrl->IGuiControl_GetPage() != navState.mSelectedPage)
        continue;
      ctrl->IGuiControl_Render(false, false, *this, mDisplay);
    }
  ...
  void DrawPageIndicators(const GuiNavigationState& navState)
  {
    ...
    if (navState.mSelectedPage < navState.mPageCount - 1) {
      mDisplay.DrawBitmap(PointI::Construct(mDisplay.ScreenSize().width - gNextPageBitmapSpec.widthPixels,
...
{
  virtual const char* DisplayAppGetName() override { return "GuiTestApp"; }

  GuiTestApp(IAppDisplay& display)
    : GuiApp(display)
  {
  }
  ...
  GuiKnobControl mKnob1 = { 1,           PointI::Construct(5, 30), StandardRangeSpecs::gFloat_N1_1,
                            "Knob1", // formatter for edit
                            floatParam1, NullBoolBinding,          AlwaysEnabled };

  float floatParam2 = 0.0f;
  GuiKnobControl mKnob2 = { 1,           PointI::Construct(55, 30), StandardRangeSpecs::gFloat_0_2,
                            "Knob2", // formatter for edit
                            floatParam2, NullBoolBinding,           AlwaysEnabled };
  ...
  virtual GuiControlList* GetRootControlList() override { return &mList; }
};
