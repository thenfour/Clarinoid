...
struct HoneywellABPI2C
{
  ...
  void Update()
  {
    ...
    if (mStatus != Status::Ok) {
      return; // under any error condition don't use the data! even under StaleData, there's no point using the
              // data.
    }
    mRawPressure = ((((uint16_t)b0) << 8) | b1) & ((1 << PressureBitsResolution) - 1);
    // convert this 14-bit value to a 0-1 float val.
    mValue = (float)mRawPressure / ((1 << PressureBitsResolution) - 1);

    mRawTemperature = ((((uint16_t)b2) << 3) | (b3 >> 5));
    mTemperatureC = (((float)mRawTemperature * 200) / 2047) - 50;
  }

  float GetPressure01() const { return mValue; }
  float GetTemperatureC() const { return mTemperatureC; }

  float GetRawPressure() const { return mRawTemperature; }
  float GetRawTemperature() const { return mRawTemperature; }

private:
  TwoWire& mWire;
  uint8_t mAddress;
  Status mStatus;
  uint16_t mRawPressure = 0;
  uint16_t mRawTemperature = 0;
  float mValue;
  float mTemperatureC;
};

struct CCHoneywellAPB : IAnalogAxis
{
  ...
  void Update() { mAbp.Update(); }

  virtual float CurrentValue01() const { return mAbp.GetPressure01(); }
};
