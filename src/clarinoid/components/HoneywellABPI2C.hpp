...
struct HoneywellABPI2C
{
    ...
    explicit HoneywellABPI2C(TwoWire &wire, uint8_t address = 0x28) : mWire(wire), mAddress(address)
    {
        wire.begin();
    }
...
struct CCHoneywellAPB : IAnalogAxis
{
    ...
    explicit CCHoneywellAPB(TwoWire &wire, uint8_t address = 0x28) : mAbp(wire, address)
    {
