...
void LoopEvent_WriteTwo12BitValues(Ptr &p, uint16_t val1, uint16_t val2)
{
    ...
    uint8_t bytes[3] = {
        ...
        (uint8_t)((uint8_t)(((val1 & 0xf) << 4)) |
                  ((uint8_t)((val2 & (uint16_t)0xf00) >> 8))), // low 4 bits of val1 + high 4 bits of val2
...
struct LoopEvent_NoteOn
{
    ...
    explicit LoopEvent_NoteOn(const MusicalVoice &mv) : mMidiNote(mv.mMidiNote), mVelocity(mv.mVelocity)
    {
    }
    void Write(Ptr &p, event_delay_t delay) const
    {
    ...
    {
        return String("note:") + (int)mMidiNote + "v:" + (int)mVelocity;
    }
};
...
struct LoopEvent_Breath
{
    ...
    explicit LoopEvent_Breath(const MusicalVoice &mv) : mBreath01(mv.mBreath01.Serialize12Bit())
    {
    }
    void Write(Ptr &p, event_delay_t delay) const
    {
...
struct LoopEvent_Pitch
{
    ...
    explicit LoopEvent_Pitch(const MusicalVoice &mv) : mPitchN11(mv.mPitchBendN11.Serialize12Bit())
    {
    }
    void Write(Ptr &p, event_delay_t delay) const
    {
...
struct LoopEvent_BreathAndPitch
{
    ...
    String ToString()
    {
        return String("b:") + mBreath01 + ", p:" + mPitchN11;
    }
};
...
struct LoopEvent_HarmPatchChange
{
    ...
    explicit LoopEvent_HarmPatchChange(const MusicalVoice &mv) : mHarmPatchId(mv.mHarmPatch)
    {
    }
    void Write(Ptr &p, event_delay_t delay) const
    {
    ...
    String ToString() const
    {
        return String("") + (int)mHarmPatchId;
    }
};
