...
struct LooperAndHarmonizer
{
    ...
    void LoopIt(const MusicalVoice &mv)
    {
        ...
        case LooperState::DurationSet:
            ...
            if (newLayer)
            { // without a layer to move to, we can't do anything. you need a "live" layer always.
    ...
    // unlikely that we will OOM. specific strategies to tighten space can be employed (first, when you wrap up
    // recording, THEN move the recorded layer to the biggest free block) but for now until that's an actual problem,
    // just eat up more buffer.
    void ClearLayer(size_t layer)
    {
        ...
        l.Stop(); // this will mark the layer free for use (after it plays its note-off) and its memory will be frozen
                  // and forgotten.
    }
    ...
    {
        uint32_t ret =
            (uint32_t)(mLoopTimer.ElapsedTime().ElapsedMillisI()); // theoretical error for very long loop times.
    ...
    size_t Update(const MusicalVoice &liveVoice,
                  ...
                  MusicalVoice *outpEnd)
    {
        ...
        */
        ...
        // Calculate the loop time for this event. Don't repeat millis() calls, and ensure the time doesn't change
        // through this processing.
        UpdateCurrentLoopTimeMS();

        // in order to feed the scale follower with a single buffer for both "live" (may not be played!) and harmonized
        // voices, go through all looper layers & harmonizer voices, and if we know the note then put it. if there are 0
        // notes output, then just put the "live" note but mark it as muted.

        MusicalVoice *pout = outp;
        MusicalVoice *pLiveVoices[LOOP_LAYERS] = {
            0}; // keep track of where i read layer state into, for later when deduced voices are filled in.
        ...
        for (uint8_t iLayer = 0; iLayer < SizeofStaticArray(mLayers); ++iLayer)
        {
            ...
            else
            {
                ...
                // consume events sequentially. even if the layer is not playing, it may send a note off. "use" the
                // layer in this case. here's a dilemma: imagine a note has a release envelope after a note-off. In
                // otherd words we cannot tell from note offs whether a voice should be active or not. in that case, the
                // synth should really be responsible for "holding" these voices on. here we just want to track whether
                // a voice is ON or has events.
            ...
            if (layerIsUsed)
            {
                ...
                pout += mHarmonizer.Harmonize(iLayer,
                                              pout,
                                              transitionEvents,
                                              pout + 1,
                                              outpEnd,
        ...
        // go through and fill in all deduced voices. logically, the "live voices" will all remain untouched so voices
        // just get filled in.
        // int deduced = 0;
        for (uint8_t iLayer = 0; iLayer < SizeofStaticArray(mLayers); ++iLayer)
        {
