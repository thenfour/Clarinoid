...
namespace clarinoid
{
static const uint32_t LOOP_BREATH_PITCH_RESOLUTION_MS =
    5; // record only every N milliseconds max. This should probably be coordinated with the similar throttler in
       // MusicalState, to make sure it plays well together
...
enum class LooperState
{
    ...
    DurationSet, // when true, it means at least 1 loop is active (i.e. mCurrentlyLiveLayer > 0), and mLoopDurationMS is
                 // valid.
};
...
struct LoopCursor
{
    ...
    // does this event contain the given loop time? if our event starts at L-50 with duration 150, then 0 is contained
    // for example.
...
struct LoopEvent_DebugUnified
{
    ...
    LoopEvent_DebugUnified(const LoopCursor &c) : mP(c.mP), mLoopTimeMS(c.mLoopTimeMS)
    {
        Ptr p = c.mP;
        uint8_t byte2;
...
struct LoopEventStream
{
    ...
    // if you're writing into the stream, and you pass the loop length, then we start tracking a complete loop worth of
    // events. this way when we stop recording, we can accurately piece together a complete loop of events in sequence.
    ...
    {
        if (mIsPlaying && mOOM)
            return "Playing but OOM!";
        if (mIsPlaying && !mOOM)
            return "Playing";
        if (mIsRecording && mOOM)
            return "Recording but OOM!";
        if (mIsRecording && !mOOM)
            return "Recording";
        if (mNeedsNoteOff)
            return "Needs note off";
        return "Idle";
    }

    const LoopStatus *mpStatus = nullptr;
    const LoopStatus &GetStatus() const
    {
        return *mpStatus;
    }
    ...
    {
        if (mOOM)
            return; // there are ways to sorta work with a OOM situation, but it's not necessary to add the complexity
                    // for a shoddy solution.
        if (!mIsPlaying)
            return;
    ...
    {
        if (mOOM)
            return false; // there are ways to sorta work with a OOM situation, but it's not necessary to add the
                          // complexity for a shoddy solution.
    ...
    void Dump()
    {
        ...
        if (mIsRecording)
        {
            ...
            {
                clarinoid::log(
                    "   > { %p pe padding %d bytes }", mBufferBegin.mP, mPrevCursor.mP.DistanceBytes(mBufferBegin.mP));
            }
        }
        else
        {
        ...
        for (auto &e : events)
        {
            ...
            if (mIsRecording && !encounteredPadding && (e.mP >= mCursor.mP))
            {
                ...
                case LayoutSituation::EP:
                    ...
                    clarinoid::log(" E > { %p t=%d EP padding %d bytes }",
                                   mCursor.mP,
                                   mCursor.mLoopTimeMS,
                                   mPrevCursor.mP.DistanceBytes(mCursor.mP));
                    break;
                }
            }
        ...
        if (mBufferBegin.mP)
        {
            clarinoid::log("B state {%s}",
                           LoopEvent_FullState(mBufferBegin.mRunningVoice).ToString().mStr.str().data());
        }
        if (mPrevCursor.mP)
        {
    ...
    Ptr WrapUpRecording()
    {
        // if we didn't record any events, this buffer is basically transparent; nothing needs to be done at all.
        ...
        if (GetLayoutSituation() == LayoutSituation::EP)
        { // |xxE---PxxxxZxx (ep)
    ...
    bool WriteEventRaw(const Tparams &e)
    {
        ...
        // this is a useful calculation to do NOW, because we know it's always less than 1 loop duration in length which
        // simplifies math.
        ...
        // advance the "prev" write cursor if it exists. do this before checking the OOB conditions below, to make a bit
        // of room and squeeze out a few more bytes.
        if (GetStatus().mState == LooperState::DurationSet)
        {
            // fast-forward P so it's just under 1 loop's worth of material.
            // it's not necessary to support a full loop because you'd end up with 2 versions of "state" at the loop
            // point (beginning + end).
    ...
    void Write(const MusicalVoice &liveVoice)
    {
        ...
        else if (transitionState.mNeedsNoteOff)
        {
            ...
            // if we actually write the note off in this case, you'll note off the new note. we don't store the note off
            // note so just skip that.
            mCursor.mRunningVoice.mMidiNote = liveVoice.mMidiNote;
            mCursor.mRunningVoice.mVelocity = liveVoice.mVelocity;
