...
static bool
GuiInitiateTooltip(bool isSelected, bool isEditing, DisplayApp& app)
{
  if (!isSelected)
    return false;
  ...
  if (!isEditing) {
    app.mDisplay.DrawMarchingAntsFilledRect(3,
                                            1,
                                            1,
                                            1,
                                            0,
                                            app.mDisplay.GetClientHeight() - app.mDisplay.GetLineHeight(),
                                            app.mDisplay.ScreenSize().width,
                                            1,
                                            0);
...
{
  String mStaticCaption;
  GuiStaticTooltipRenderer(const String& staticCaption)
    : mStaticCaption(staticCaption)
  {
  }
  virtual void IGuiRenderer_Render(IGuiControl& ctrl,
...
struct GuiNumericEditor : IGuiEditor<Tparam>
{
  ...
  GuiNumericEditor(const NumericEditRangeSpec<Tparam>& range)
    : mRange(range)
  {
  }
...
struct GuiRectangleMarquee : IGuiControl
{
  ...
  GuiRectangleMarquee(int page, const RectI& bounds, Edges::Flags edges = Edges::All)
    : mEdgeFlags(edges)
  {
    this->mPage = page;
    this->mBounds = bounds;
