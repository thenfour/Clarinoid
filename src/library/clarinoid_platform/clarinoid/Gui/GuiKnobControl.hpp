...
struct GuiKnobRenderer : IGuiRenderer<float>
{
  ...
  GuiKnobRenderer(const NumericEditRangeSpec<float>& range)
    : mRange(range)
  {
  }
  ...
  {
    app.mDisplay.DrawBitmap(ctrl.mBounds.TopLeft(), KnobDetails::gKnobOutlineSpec);
    ...
    app.mDisplay.FillPie(
      KnobDetails::Origin.WithOffset(ctrl.mBounds.TopLeft().Cast<float>()), KnobDetails::Radius, a0, a1 - a0);
  }
};
...
struct GuiGainKnobRenderer : IGuiRenderer<float>
{
  ...
  GuiGainKnobRenderer(const NumericEditRangeSpecWithBottom& range)
    : mRange(range)
  {
  }
  virtual void IGuiRenderer_Render(IGuiControl& ctrl,
                                   ...
                                   DisplayApp& app) override
  {
    ...
    if (mRange.IsBottom(valDb)) {
      // -inf db,
      app.mDisplay.DrawBitmap(ctrl.mBounds.TopLeft(), KnobDetails::gKnobOutlineMutedSpec);
      return;
    }
    ...
    if (mRange.mRangeMax >= 0) // user can GAIN in addition to attenuation; put zero point at the top.
    {
      ...
      app.mDisplay.FillPie(
        KnobDetails::Origin.WithOffset(ctrl.mBounds.TopLeft().Cast<float>()), KnobDetails::Radius, a0, aCenter - a0);
      return;
    }
...
struct GuiLabelGainTooltipRenderer : IGuiRenderer<T>
{
  ...
  GuiLabelGainTooltipRenderer(const String& staticCaption)
...
struct GuiGainEditor : IGuiEditor<float>
{
  ...
  GuiGainEditor(const NumericEditRangeSpecWithBottom& range)
    : mRange(range)
  {
  }
