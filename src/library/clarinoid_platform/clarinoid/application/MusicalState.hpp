...
int
KeyStateToRelativeNote(bool LH1, bool LH2, bool LH3, bool LH4, bool RH1, bool RH2, bool RH3, bool RH4)
{
  int relativeNote = 0;
  if (LH1)
  ...
  if (RH3) {
    ...
    // Normally you want to use LH4 as a +1 key, but when it's down you often still want to +1 (Eb - E or Db - D). it's
    // natural to use rh2 for this. coming from G# 1 000 -> 001 this is -1 F# coming from G  1 010 -> 011 this is -1 F
    // <-- allows rh2 trill between F and F# coming from F# 1 100 -> 101 this is -1 E coming from F  1 110 -> 111 this
    // is -2 D
    relativeNote--;
    ...
    // the ugly thing about enabling those trills is that now LH4 no longer acts as a +1, but i think it's an acceptable
    // compromise.
  }
  if (RH4)
    relativeNote -= 2;
...
struct CCEWIMusicalState
{
  ...
  MusicalVoice mMusicalVoices[MAX_MUSICAL_VOICES]; // these are all the voices that WANT to be played (after transpose,
  ...
  CCEWIMusicalState(IAppDisplay* pDisplay,
  ...
  void Update()
  {
    ...
    if (mMetronomeEnableToggleReader.IsNewlyPressed()) {
      ...
      mAppSettings->mMetronomeLED = !mAppSettings->mMetronomeLED;
      mInputSrc->InputSource_ShowToast(String("Metronome: ") + ((mAppSettings->mMetronomeSoundOn) ? "on" : "off"));
    }
