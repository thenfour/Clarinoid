...
{
  virtual const char* DisplayAppGetName() override { return "PerformancePatchSettingsApp"; }

  PerformancePatch& GetBinding() { return mAppSettings->GetCurrentPerformancePatch(); }

  PerformancePatchSettingsApp(IAppDisplay& d)
    : SettingsMenuApp(d)
  {
  }
  ...
  EnumSettingItem<GlobalScaleRefType> mGlobalScaleRef = { "Scale ref",
                                                          gGlobalScaleRefTypeInfo,
                                                          Property<GlobalScaleRefType>{
                                                            [](void* cap) FLASHMEM {
                                                              auto* pThis = (PerformancePatchSettingsApp*)cap;
  ...
  EnumSettingItem<ClarinoidFilterType> mDelayFilterType = { " >Filter",
  ...
  ISettingItem* mMasterFXSubmenuItems[9] = {
    &mReverbDamping,   &mReverbSize,           &mDelayTimeMS,     &mDelayStereoSep, &mDelayFeedbackLevel,
    &mDelayFilterType, &mDelayCutoffFrequency, &mDelaySaturation, &mDelayQ,
  };
  ...
  ISettingItem* mArray[14] = {
    &mMasterGain,          &mTranspose,

    &mGlobalScaleRef,      &mDeducedScale,        &mChosenScaleNote, &mChosenScaleFlavor,

    &mSelectedSynthPatchA, &mSelectedSynthPatchB, &mStereoSpread,    &mSelectedHarmPatch,
    &mMasterFXEnable,      &mReverbGain,          &mDelayGain,       &mMasterFX,
  };
  ...
  {
    mDisplay.ClearState();
    mDisplay.PrintLine(String("Performance > "));
    mDisplay.PrintLine(GetAppSettings()->GetPerfPatchName(GetAppSettings()->mCurrentPerformancePatch));
