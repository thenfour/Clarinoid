...
// the system works like this: each app has a front page which you scroll through using the encoder. rendered using
// RenderFrontpage(). clicking the encoder will enter the app, rendered with Render().
struct DisplayApp : IDisplayApp
{
  ...
  virtual void DisplayAppOnSelected() override
  {
    ...
    mEnc.ClearState(); // Necessary because we haven't been tracking state while our app is not selected. So if you
                       // select this app your encoder will read a huge delta and switch immediately to some other
                       // app.
    // Serial.println(String("cleared state ; delta=") + mEnc.GetIntDelta());
    GoToFrontPage();
  }

  void GoToFrontPage() { mShowingFrontPage = true; }
