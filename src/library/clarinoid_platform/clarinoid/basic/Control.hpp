...
struct ControlValue
{
  ...
  public:
    ...
    {
        return FloatEquals(
            mVal, f, 0.0001f); // seems reasonable. a 16-bit input would be a precision of .0001525902189669642
    }
...
struct ControlReader
{
  ...
  public:
    ...
    {
        return mCurrentValue.AsBool();
    }
    bool IsCurrentlyUnpressed() const
    {
        return !IsCurrentlyPressed();
    }
    bool IsNewlyPressed() const
    {
        return mCurrentValue.AsBool() && mDirty;
    }
    bool IsNewlyUnpressed() const
    {
        return !mCurrentValue.AsBool() && mDirty;
    }
    bool IsDirty() const
    ...
    float GetCurrentFloatValue01() const
    {
        return mCurrentValue.AsFloat01();
    }
    float GetPreviousFloatValue01() const
    {
        return mPreviousValue.AsFloat01();
    }

    // assuming encoder-like,
    int GetIntDelta() const
    {
        return mCurrentValue.AsFlooredInt() - mPreviousValue.AsFlooredInt();
    }
    float GetFloatDelta() const
    {
        return mCurrentValue.AsFloat01() - mPreviousValue.AsFloat01();
    }
    int GetIntValue() const
    {
        return mCurrentValue.AsFlooredInt();
    }
...
struct SwitchControlReader
{
  ...
  public:
    ...
    bool IsUnpressedState() const
    {
        return !mCurrentValue;
    }
    bool IsNewlyPressed() const
    {
        return mCurrentValue && mDirty;
    }
    bool IsNewlyUnpressed() const
    {
        return !mCurrentValue && mDirty;
    ...
    bool WasPressedState() const
    {
        return mPreviousValue;
    }
    bool WasUnpressedState() const
    {
        return !mPreviousValue;
    }
};
  ...
  public:
    ...
    void Update(IEncoder *enc)
    {
        if (mC != enc)
        {
            ...
            mCurrentValue = mC->GetControlValue();
            mPreviousValue = mCurrentValue;
            mDirty = false; // since current == prev, plus we don't want everything to act like you just changed stuff
                            // at the startup.
            // mFirst = false;
            return;
        }
