...
struct FunctionListEditor : ISettingItemEditor
{
  struct ListProxy : IList
  {
    ...
    virtual int List_GetItemCount() const override { return mItemCount; }
    virtual String List_GetItemCaption(int i) const override { return mItemNameGetter(mCap, (size_t)i); }
  };
  ...
  virtual void Render()
  {
    ...
    int nitems = mpApi->GetDisplay()->GetClipRect().Height() / mpApi->GetDisplay()->GetLineHeight();
