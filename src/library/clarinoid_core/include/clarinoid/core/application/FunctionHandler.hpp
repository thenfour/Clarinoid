...
struct FunctionHandler
{
    ...
    virtual void Update(ControlMapping &mapping, const ControlInfo &src)
    {
        ...
        mAggregateValue = ControlMapping::ApplyValue(
            &mAggregateValue,
            out,
            mapping.mOperator); // FunctionHandler_ApplyValue(mAggregateValue, out, mapping.mOperator);
    }
...
struct VirtualAxis : IAnalogAxis, FunctionHandler
{
    ...
    virtual ControlValue FunctionHandler_GetCurrentValue() const override
    {
        return ControlValue::FloatValue(mValue);
    }
};
