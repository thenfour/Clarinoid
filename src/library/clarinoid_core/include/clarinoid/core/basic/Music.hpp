...
// asinine.
inline float
MIDINoteToFreq(float x)
{
  float a = 440;
  const float oneThirtySecond = 1.0f / 32.0f;
  const float oneTwelfth = 1.0f / 12.0f;
  return (a * oneThirtySecond) * fast::pow(2.0f, ((x - 9.0f) * oneTwelfth));
}
...
public:
  ...
  const char* ToString() const { return gNotes[mNoteIndex].mNameWithCustomGlyphs; }
...
struct ScaleFlavor
{
    ...
    , howCommonInScaleDetector(howCommonInScaleDetector)
  {
    ...
    // we need to find a way to normalize fitness score so every scale flavor will return a similar value for similar
    // fitness.
  ...
  uint8_t NormalizeScaleDegree(int8_t s) const { return RotateIntoRangeByte(s, (uint8_t)mIntervalCount); }
...
struct PitchFitnessClass
{
  ...
  static constexpr float NotInScale_Agnostic = 0; // not in scale, but does not penalize. rare to use this; only place i
                                                  // could really think needs this is minor blues around the 7th.
...
struct Scale
{
  ...
  bool operator!=(const Scale& rhs) const { return !((*this) == rhs); }

  const ScaleFlavor& GetScaleFlavor() const { return gScaleFlavors[(size_t)mFlavorIndex]; }
  ...
  std::vector<std::pair<Note, int>> GetDiatonicNotesAndCharacter() const
  {
    ...
    for (Note n = (Note)0; (int)n < 12; n = (Note)((int)n + 1)) {
      ...
      if (ctx.mEnharmonic == 0) {
        ...
        ret.push_back(std::make_pair(n, this->GetScaleFlavor().mDegreeCharacteristicStrengths[ctx.mScaleDegree]));
      }
    }
    return ret;
  }
  ...
  String ToString() const { return String(gNotes[(uint8_t)mRootNoteIndex].mName) + " " + GetScaleFlavor().mLongName; }
};
