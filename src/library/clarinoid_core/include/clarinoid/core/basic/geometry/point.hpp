...
struct Point
{
  ...
  static Point Construct(T x_, T y_)
  {
    return {x_, y_};
  }
  ...
  Point WithY(T y_) const
  {
    return {x, y_};
  }
  Point WithXOffset(T dx) const
  {
    return {x + dx, y};
  }
  Point WithYOffset(T dy) const
  {
    return {x, y + dy};
  }
  Point WithOffset(T dx, T dy) const
  {
    return {x + dx, y + dy};
  }
  Point WithOffset(const Point<T>& pt) const
  {
