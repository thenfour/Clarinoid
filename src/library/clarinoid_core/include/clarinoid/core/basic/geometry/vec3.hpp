...
struct Vec3
{
  ...
  [[nodiscard]] Vec3 reject_from(const Vec3& v) const noexcept
  {
  ...
  Vec3& rotate_about_axis(const Vec3& axis, T radians) noexcept
  {
    return (*this = rotated_about_axis(axis, radians));
  }
  ...
  [[nodiscard]] constexpr std::array<T, 3> to_array() const noexcept
  {
    return {x, y, z};
  }
