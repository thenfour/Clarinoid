...
template <typename T, size_t N>
inline constexpr size_t SizeofStaticArray(const T (&x)[N])
{
  return N;
}

template <typename T, size_t N>
inline void CopyPODArray(const T (&from)[N], T (&to)[N])
{
...
template <typename T>
inline void CopyPODArray(const T* from, T* to, size_t N)
{
  memcpy(to, from, sizeof(T) * N);
}

// span is C++20, array_view was never adopted.
template <typename T>
struct array_view
{
  size_t mSize = 0;
...
template <typename T, size_t N>
inline array_view<T> make_array_view(T (&a)[N])
{
