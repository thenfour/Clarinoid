...
struct IGuiControl
{
  ...
  IGuiControl() {}
  explicit IGuiControl(int page, const RectI& bounds)
    : mPage(page)
    , mBounds(bounds)
  {
  }
  virtual int IGuiControl_GetPage() { return mPage; }
  virtual RectI IGuiControl_GetBounds() { return mBounds; }
  virtual bool IGuiControl_IsSelectable() { return mIsSelectable; }
  ...
  virtual void IGuiControl_Render(bool isSelected, bool isEditing, DisplayApp& app, IAppDisplay& display) = 0;
  virtual void IGuiControl_Update(bool isSelected, bool isEditing, DisplayApp& app, IAppDisplay& display) = 0;
};
...
struct GuiControlList // very much like SettingsList. simpler though because we don't bother with multi items.
{
  ...
  template<size_t N>
  GuiControlList(IGuiControl* (&arr)[N])
    : mItems(arr)
    , mItemRawCount(N)
  {
  }

  size_t Count() const { return mItemRawCount; }
...
struct GuiLabelControl : IGuiControl
{
  ...
  String getText() const { return mTextFn ? mTextFn(mCapture) : mText; }
  ...
  virtual void IGuiControl_Render(bool isSelected, bool isEditing, DisplayApp& app, IAppDisplay& display) override
  {
    display.ClearState();
    display.SetTextWrap(false);
    display.SetCursor(mBounds.TopLeft());
    if (mClip) {
      display.SetClipRect(mBounds);
    }
    display.Print(getText());
  }
  virtual void IGuiControl_Update(bool isSelected, bool isEditing, DisplayApp& app, IAppDisplay& display) override {}
};
...
struct GuiCompositeControl : IGuiControl
{
  ...
  virtual void IGuiControl_Render(bool isSelected, bool isEditing, DisplayApp& app, IAppDisplay& display) override
  {
    display.ClearState();
    display.SetCursor(mBounds.TopLeft());
    mRenderCtrl->IGuiRenderer_Render(*this, mBinding.GetValue(), mDblBinding.GetValue(), isSelected, isEditing, app);
  ...
  virtual bool IGuiControl_IsSelectable() override { return mIsSelectable.GetValue(); }
};
...
struct GuiRendererCombiner : IGuiRenderer<T>
{
  ...
  GuiRendererCombiner(IGuiRenderer<T>* a, IGuiRenderer<T>* b)
    : mRenderer1(a)
    , mRenderer2(b)
  {
  }
