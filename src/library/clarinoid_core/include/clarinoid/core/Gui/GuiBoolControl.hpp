...
struct GuiMuteControlRenderer : IGuiRenderer<bool>
{
                                   ...
                                   DisplayApp& app) override
  {
    ...
    app.mDisplay.DrawBitmap(ctrl.mBounds.TopLeft(), val ? gMuteOnBitmapSpec : gMuteOffBitmapSpec);
...
struct GuiPatchMuteControlRenderer : IGuiRenderer<bool>
{
  ...
  {
    bool val = (invertVal ? !val_ : val_);
    app.mDisplay.DrawBitmap(ctrl.mBounds.TopLeft(), val ? gPatchEnabledSpec : gPatchDisabledSpec);
    if (GuiInitiateTooltip(isSelected, isEditing, app)) {
      app.mDisplay.Print(mStaticCaption + ": " + (val_ ? mTrueValue : mFalseValue));
    }
  }
