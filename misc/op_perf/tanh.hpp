...
static constexpr int64_t E_Q31 = 0x1A0DA3A6LL;
    ...
    int64_t  num =  x  * (  A_Q31 + ((A_Q31 * ax64) >> 31)
                     +  ( (B_Q31 + ((C_Q31 * ax64) >> 31)) * x2 >> 31) );
    // denominator
    ...
    // saturation
    if (y64 >  0x7FFFFFFFLL) y64 = 0x7FFFFFFFLL;
    if (y64 < -0x80000000LL) y64 = -0x800000000LL;
    return (int32_t)y64;
}
