...
{
  Harmonizer harm;
  MusicalVoice mv[HARM_VOICES + 1];
  MusicalVoice& lv(mv[0]);
  MusicalVoiceTransitionEvents te;
  gAppSettings.mGlobalScaleRef = GlobalScaleRefType::Chosen;
  gAppSettings.mGlobalScale = Scale{ Note::E, ScaleFlavorIndex::MajorPentatonic };
  //gAppSettings.mGlobalHarmPreset = 1;
  lv.mHarmPatch = 1;

  // use preset #1.
  auto& preset = gAppSettings.mHarmSettings.mPresets[1];
  ...
  lv.mVelocity = 50;
  lv.mHarmPatch = 1;
  size_t vc = harm.Harmonize(2, &lv, te, mv + 1, EndPtr(mv), Harmonizer::VoiceFilterOptions::AllExceptDeducedVoices);
  Test(vc == 1);
  Test(lv.mIsNoteCurrentlyMuted = true);
  Test(lv.mVoiceId == MakeMusicalVoiceID(2, 0));
  ...
  lv.mVelocity = 50;
  lv.mHarmPatch = 1;
  vc = harm.Harmonize(2, &lv, te, mv + 1, EndPtr(mv), Harmonizer::VoiceFilterOptions::AllExceptDeducedVoices);
  Test(vc == 1);
  ...
  preset = HarmPreset();
  preset.mVoiceSettings[0].mScaleRef = HarmScaleRefType::Voice;
  ...
  Test(mv[1].mVoiceId == MakeMusicalVoiceID(2, 1));

  // global scale usage { E major pentatonic -2} 
  preset = HarmPreset();
  preset.mVoiceSettings[0].mScaleRef = HarmScaleRefType::Global;
  ...
  preset.mVoiceSettings[1].mScaleRef = HarmScaleRefType::Global;
  ...
  Test(mv[2].mVoiceId == MakeMusicalVoiceID(2, 2));
