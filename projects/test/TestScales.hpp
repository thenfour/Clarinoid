...
//using clarinoid::ScaleFlavorIndex;
//using clarinoid::MidiNote;
...
void TestScaleFlavors()
{
    ...
    int8_t offsetAdj = 0;
    Test(sfhmin.ContextToChrRelativeNote(clarinoid::NoteInScaleFlavorContext{2, -1}, offsetAdj) ==
         2); // scale degree 2 = Eb, flat = D = 2.
    Test(sfhmin.ContextToChrRelativeNote(clarinoid::NoteInScaleFlavorContext{1, -1}, offsetAdj) ==
         1); // scale degree 1 = D, flat = Db = 1.
...
void TestScales()
{
    ...
    MidiNote m {3, Note::E};                                                                   // oct3, E
    MidiNote m2{s.AdjustNoteByInterval(m.GetMidiValue(), 1, EnharmonicDirection::Sharp)};  // should go to F#
    Test(m2.GetOctave() == 3);
    Test(m2.GetNoteIndex() == 6); // F#

    m2 = MidiNote{s.AdjustNoteByInterval(m.GetMidiValue(), -3, EnharmonicDirection::Sharp)}; // should go to F#
    Test(m2.GetOctave() == 2);
