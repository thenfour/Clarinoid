...
struct Clarinoid2LedsTask : Leds<28 /*ledcount*/, 14 /*pin*/>, ITask
{
    ...
    PeakMeter<8 /* # leds */, 2500 /*holdtime*/, 200 /*fallofftime*/> mPeakMeter;
    ...
    virtual void TaskRun() override
    {
        ...
        for (size_t i = 0; i < 28; ++i)
        {
            ...
            {
                this->SetPixel(i, ((serialCount & (1 << i)) > 0) ? 4 : 0, 0, 0);
            }
            else if (i > 1 && i < 6)
            {
                this->SetPixel(i, 0, 0, ((serialCount & (1 << i)) > 0) ? 14 : 0);
            }
        }
        ...
        if (mpProvider->ILEDDataProvider_GetMusicalState()->mAppSettings->mMetronomeLED)
        {
            ...
            {
                this->SetPixel(metronomeLEDIndex,
